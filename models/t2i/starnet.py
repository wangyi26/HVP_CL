...
from .vit import Block as ViTBlock
...
class StarNet(nn.Module):
    def __init__(self, base_dim=32, depths=[3, 3, 12, 5], mlp_ratio=4, drop_path_rate=0.0, num_classes=-1, 
                 out_dims=768, pretrained=None, **kwargs):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = self.norm(x)
        b,c,h,w = x.shape
        cls_token = torch.flatten(self.avgpool(x), 1)
        cls_token = cls_token.reshape(-1, 1, c)
...
class KDStarNet(StarNet):
    def __init__(self, base_dim=32, depths=[3, 3, 12, 5], mlp_ratio=4, drop_path_rate=0.0, num_classes=-1, 
                 out_dims=768, pretrained=None, **kwargs):
        super().__init__(base_dim, depths, mlp_ratio, drop_path_rate, num_classes, out_dims, pretrained=None, **kwargs)
        ...
        scale = 192 ** -0.5 
        self.proj = nn.Parameter(scale * torch.randn(192, out_dims))
        self.apply(self._init_weights)
        if pretrained:
            self.init_weights_from_pretrained(pretrained)
...
def starnet_s3(pretrained=False, **kwargs):
    model = StarNet(32, [2, 2, 8, 4], 
                    pretrained='/mnt/hdd3/wangxuanhan/research/vfm_research/learning_research/mae-dev/models/lupsub_csl_csm_css_starnet_s3.pth', 
                    **kwargs)
    if pretrained:
...
def kd_starnet_s3(pretrained=False, **kwargs):
    model = KDStarNet(32, [2, 2, 8, 4], 
                      pretrained='./pretrained/lupsub_csm_kd_starnet_s3.pth',
                    #pretrained='/mnt/hdd3/wangxuanhan/research/vfm_research/learning_research/mae-dev/models/lupsub_csm_kd_starnet_s3.pth', 
                    **kwargs)
    return model
