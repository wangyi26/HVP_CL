...
class SwinTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        scale = (8*embed_dims) ** -0.5 
        self.proj = nn.Parameter(scale * torch.randn(8*embed_dims, out_dims))
        if pretrained:
    ...
    def init_weights(self, pretrained=None):
        ...
        if pretrained is None:
            ...
            if 'state_dict' in ckpt:
                ...
                _state_dict = ckpt
            _state_dict = {k.replace("backbone.", ""): v for k, v in _state_dict.items()}
            _state_dict = {k.replace("module.", ""): v for k, v in _state_dict.items()}
...
def swin_small_patch4_window7_224(img_size=224,drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0., **kwargs):
    model = SwinTransformer(pretrain_img_size = img_size, patch_size=4, window_size=7, embed_dims=96, depths=(2, 2, 18, 2), num_heads=(3, 6, 12, 24), drop_path_rate=drop_path_rate, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, 
                            pretrained='/home/wangxuanhan/data/research/vfm_research/learning_research/saipv2/work_dirs/lup1m_solider-b_to_swin_small_from_cls_patch_moe/checkpoint.pth',
                            **kwargs)
    return model

def swin_tiny_patch4_window7_224(img_size=224,drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0., **kwargs):
    model = SwinTransformer(pretrain_img_size = img_size, patch_size=4, window_size=7, embed_dims=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), drop_path_rate=drop_path_rate, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, 
                            pretrained='/mnt/hdd4/wangxuanhan/saipv2_models/lup1m_path-b_to_swin_t/checkpoint.pth',
                            **kwargs)
    return model
