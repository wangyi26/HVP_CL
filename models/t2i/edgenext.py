...
class EdgeNeXt(nn.Module):
    def __init__(self, in_chans=3, num_classes=-1,
        ...
        scale = dims[-1] ** -0.5 
        self.proj = nn.Parameter(scale * torch.randn(dims[-1], out_dims))
    ...
    def forward(self, x):
        ...
        x = feats[-1]
        b,c,h,w = x.shape
        cls_token = self.norm(feats[-1].mean([-2, -1]))
...
def edgenext_small(pretrained=False, **kwargs):
    ...
    model = EdgeNeXt(depths=[3, 3, 9, 3], dims=[48, 96, 160, 304], expan_ratio=4,
                     ...
                     kernel_sizes=[3, 5, 7, 9],
                     d2_scales=[2, 2, 3, 4],
                     pretrained='/mnt/hdd3/wangxuanhan/research/vfm_research/learning_research/mae-dev/models/imagenet1k/edgenext_small.pth',
                     **kwargs)
