...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, 
                ...
                attn_drop_rate=0., camera=0, view=0,drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6), 
                local_feature=False, sie_xishu =1.0, hw_ratio=1, gem_pool = False, stem_conv=False, pretrained=None,**args):
        super().__init__()
        ...
        # print(self.gem_pool,self.cam_num,self.view_num,self.local_feature)
        if self.gem_pool:
    ...
    def load_param(self, model_path,hw_ratio):
        ...
        if 'student' in param_dict: ### for dino
            obj = param_dict["student"]
...
class KDTransReID(TransReID):
    """ Transformer-based Object Re-Identification
    """
    def __init__(self, embed_dim=768, 
                depth=12,num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., 
                attn_drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6), 
                num_heads_in_last_block=12, pretrained=None, frozen_stages=-1, **args):
        ...
        # self.adapter = AlignDecoder(embed_dim)
        ...
        if self.frozen_stages>0:
            print('frozen {} stages'.format(self.frozen_stages))
            self.freeze_stages()
...
class AlignTransReID(TransReID):
    """ Transformer-based Object Re-Identification
    """
    def __init__(self, embed_dim=768, 
                depth=12,num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., 
                attn_drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6), 
                pretrained=None, frozen_stages=-1, **args):
        super().__init__(embed_dim=embed_dim, 
                depth=depth,num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop_rate=drop_rate, 
                attn_drop_rate=attn_drop_rate, drop_path_rate=drop_path_rate, norm_layer=norm_layer, 
                pretrained=None, **args)
        ...
        self.adapter = ViTAdapter(embed_dim)
...
def kd_vit_small_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    print('kd vit small')
    model = KDTransReID(img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=384, depth=12, num_heads=6, num_heads_in_last_block=12, mlp_ratio=4, qkv_bias=True,drop_path_rate=drop_path_rate, camera=camera, view=view, sie_xishu=sie_xishu, local_feature=local_feature,  **kwargs)
    model.in_planes = 384
    return model
...
def kd_vit_tiny_patch16_224_TransReID(img_size=(252, 126), stride_size=16, drop_path_rate=0.1, camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    print('kd vit tiny')
    # model = KDTransReID(img_size=img_size, patch_size=14, stride_size=stride_size, embed_dim=192, depth=12, num_heads=6, num_heads_in_last_block=12, mlp_ratio=4, qkv_bias=True,drop_path_rate=drop_path_rate, camera=camera, view=view, sie_xishu=sie_xishu, local_feature=local_feature,  frozen_stages=-1, **kwargs)
    # model.in_planes = 192
    # return model
    model = AlignTransReID(img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=192, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,drop_path_rate=drop_path_rate, camera=camera, view=view, sie_xishu=sie_xishu, local_feature=local_feature,  frozen_stages=-1, **kwargs)
    model.in_planes = 768
    return model
