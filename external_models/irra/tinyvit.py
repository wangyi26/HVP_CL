...
class TinyViT(nn.Module):
    def __init__(self, img_size=[224,224], in_chans=3,
        ...
        self.final_dims = 256
        scale = self.final_dims ** -0.5 
        self.proj = nn.Parameter(scale * torch.randn(self.final_dims, out_dims))
        self.apply(self._init_weights)
        if pretrained:
    ...
    def random_masking(self, x, keep_ratio=(0.5, 0.5)):
        """
        ...
        keep_r_h, keep_r_w = keep_ratio
        keep_len_h = int(H*keep_r_h)
        keep_len_w = int(W*keep_r_w)
        mask_h = np.zeros(keep_len_h)
        mask_w = np.zeros(keep_len_w)
        keep_idx_h = np.random.choice(range(H), keep_len_h, replace=False)
        keep_idx_h.sort()
        keep_idx_w = np.random.choice(range(W), keep_len_w, replace=False)
        keep_idx_w.sort()
        mask_h[keep_idx_h] += 1.
        mask_w[keep_idx_w] += 1.
        keep_idx_2d = mask_h.unsqueeze(1)*mask_w.unsqueeze(0)
        masked_idx_2d = 1. - keep_idx_2d
        x_kept = x[:, :, keep_idx_2d==1]
        x_kept = x_kept.reshape(N, D, keep_len_h, keep_len_w)
        x_masked = x[:, :, masked_idx_2d==1]

        return x_kept, x_masked, keep_idx_2d
    ...
    def forward(self, x):
        ...
        x = outs[-1]
        b,c,h,w = x.shape
        cls_token = self.avgpool(outs[-1])
        cls_token = torch.flatten(cls_token, 1).reshape(-1, 1, c)
...
class KDTinyViT(TinyViT):
    def __init__(self, img_size=[224,224], in_chans=3,
        ...
        super().__init__(img_size, in_chans,
                 embed_dims, depths,
                 num_heads,
                 window_sizes,
                 mlp_ratio,
...
class CSMKDTinyViT(TinyViT):
    def __init__(self, img_size=[224,224], in_chans=3,
        ...
        self.cls_token = nn.Parameter(torch.rand(1, 1, 192))
        self.vit_block = nn.ModuleList([Block(dim=192, num_heads=6, mlp_ratio=4, qkv_bias=True, qk_scale=None, norm_layer=nn.LayerNorm),
                                        Block(dim=192, num_heads=12, mlp_ratio=4, qkv_bias=True, qk_scale=None, norm_layer=nn.LayerNorm)])
        self.norm = nn.LayerNorm(192)
        self.stage_dims = embed_dims
        self.final_dims = 192
        scale = self.final_dims ** -0.5 
...
def tinyvit_5m(**kwargs):
    ...
    model = TinyViT(in_chans=3,
                    ...
                    local_conv_size=3,
                    layer_lr_decay=0.8,
                    pretrained='/mnt/hdd3/wangxuanhan/research/vfm_research/learning_research/mae-dev/work_dirs/in1k_lupsub_csl_tinyvit_5m_100e/checkpoint.pth',
                    **kwargs
                )

    return model

def kd_tinyvit_5m(**kwargs):

    model = KDTinyViT(in_chans=3,
                    embed_dims=[64, 128, 160, 320],
                    depths=[2, 2, 6, 2],
                    num_heads=[2, 4, 5, 10],
                    window_sizes=[7, 7, 14, 7],
                    ...
                    local_conv_size=3,
                    layer_lr_decay=0.8,
                    #pretrained='/mnt/hdd3/wangxuanhan/research/vfm_research/learning_research/mae-dev/work_dirs/lupsub_csl_kd_300e_tinyvit5m/checkpoint.pth',
                    pretrained='./pretrained/lupsub_csm_kd_tinyvit_5m.pth',
                    **kwargs
                )

    return model

def csm_kd_tinyvit_5m(**kwargs):

    model = CSMKDTinyViT(in_chans=3,
                    embed_dims=[64, 128, 160, 320],
                    depths=[2, 2, 6, 2],
                    num_heads=[2, 4, 5, 10],
                    window_sizes=[7, 7, 14, 7],
                    ...
                    local_conv_size=3,
                    layer_lr_decay=0.8,
                    pretrained='/mnt/hdd3/wangxuanhan/research/vfm_research/learning_research/mae-dev/models/lupsub_csm_kd_tinyvit_5m.pth',
                    **kwargs
                )
